...
const API_KEY = "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";
...
const CrisisConnectBot = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      message: "Hello, I'm CrisisConnectGPT! Ask me anything related to crisis or disaster management.",
      sentTime: "just now",
            ...
            <>
              <Box sx={{ maxHeight: 300, overflowY: 'auto', my: 1 }}>
                {messages.map((message, index) => (
                  <Typography key={index} color={message.sender === 'CrisisConnectGPT' ? 'primary' : 'textSecondary'}>
              ...
              <Box sx={{ display: 'flex', mt: 2 }}>
                ...
                />
                <Button variant="contained" onClick={() => handleSend(inputMessage)} sx={{ ml: 1 }}>Send</Button>
              </Box>
            </>
          )}
        </StyledPaper>
